...
import string
import seaborn as snsimport streamlit as st
import pandas as pd
import numpy as np
from wordcloud import WordCloud, STOPWORDS
import matplotlib.pyplot as plt
...
if uploaded_file is not None:
    try:
        ...
        #Plot 7: Proportion of Sentiment
        st.markdown("<h3 style = 'text-align: center;'>Proportion of Sentiment</h3>",unsafe_allow_html=True)
        ...
        if 'clean_tweets' in cld_df.columns:
            # Join all positive sentiment tweets into a single string
            positive_words = ' '.join(cld_df['clean_ttweets'][cld_df['Sentiment'] == 1].dropna().astype(str).tolist())
